...
class SessionService:
    ...
    def get_or_create_session(self) -> QuizSession:
        quiz = Quiz.custom_objects.is_public().by_uuid(self.course_uuid, self.quiz_uuid)
        if quiz is None:
            raise ValueError
        session = QuizSession.custom_objects.get_or_create_session(
            user = self.user,
            quiz = quiz
        )
        return session
    ...
    def get_session(self) -> QuizSession:
        ...
        session = QuizSession.custom_objects.get_session(
            user = self.user,
            quiz = self.quiz
        )
        return session
